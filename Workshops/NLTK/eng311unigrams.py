...
raw = newcorpus.raw()
documents = nltk.word_tokenize(raw)
docfd = nltk.FreqDist(documents)

for d in sorted(docfd.items(),key=lambda t: (-t[1], t[0]))[::]:
    print(d)

def raw_csv_writer():
    with open('PleaseRenameThisNewUnigramRaw.csv', 'w',newline='') as csvfile:
        writer = csv.writer(csvfile,delimiter=',')
        for k in sorted(docfd.items(), key=lambda t: (-t[1], t[0]))[::]:
            #print(k)
            writer.writerow(k)

raw_csv_writer()

dictionary = dictionary.Dictionary(documents)
...
def csv_writer():
    with open('documents/oa/PleaseRenameThisNewlyProducedFile.csv', 'w',newline='') as csvfile:
        writer = csv.writer(csvfile,delimiter=',')
